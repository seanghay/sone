...
export function qr(input, { version = -1, correction = "M" } = {}) {
  ...
  /** @type {number[]} */
  const cache = create_data(version, error_correction_level, data);
...
class QrBitBuffer {
   ...
   */
  put_bit(bit) {
    const buf_index = Math.floor(this._length / 8);
    if (this._buffer.length <= buf_index) {
      this._buffer.push(0);
    }

    if (bit) {
      this._buffer[buf_index] |= 0x80 >>> (this._length % 8);
    }
...
/**
 * Create QRCode as SVG String
 *  @param {object} option
 *  @param {string} option.data
 *  @param {"M" | "L" | "H" | "Q"} option.ecl
 *  @param {number} option.margin
 *  @param {number} option.size
 *  @param {string | null} option.color
 *  @param {string | null} option.background
 *  @return {string}
 */
export function qrcodeAsSvg(_option = {}) {
  const option = {
    size: 512,
    margin: 15,
    ecl: "M",
  ...
  const modules = qr(option.data, { version: -1, correction: option.ecl });
  const margin = lerp(0, modules.length, option.margin / 100);
  const blockSize = option.size / (modules.length + 2 * margin);

  let pathdata = "";
  for (let y = 0; y < modules.length; y++) {
    for (let x = 0; x < modules.length; x++) {
      const _module = modules[x][y];

      if (_module) {
        let px = x * blockSize + margin * blockSize;
        let py = y * blockSize + margin * blockSize;
  ...
  const modrect = `<path x="0" y="0" style="fill:${option.color};shape-rendering:crispEdges;" d="${pathdata}" />`;

  const blocks = [
    '<?xml version="1.0" standalone="yes"?>',
    `<svg xmlns="http://www.w3.org/2000/svg" version="1.1" width="${option.size}" height="${option.size}">`,
    `<defs><path id="qrmodule" d="M0 0 h${blockSize} v${blockSize} H0  H0 z" style="fill:${option.color};shape-rendering:crispEdges;" /></defs>`,
    option.background != null
      ? `<rect x="0" y="0" width="${option.size}" height="${option.size}" style="fill:${option.background};shape-rendering:crispEdges;"/>`
      : "",
    modrect,
    "</svg>",
  ];
 ...
 *  @return {Image}
 */
export function qrcode(data, option = {}) {
  const svg = qrcodeAsSvg({ data, ...option });
